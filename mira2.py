...
# Page Config
st.set_page_config(
    page_title="Mira 2.0 - WBP Assertion Viewer",
    page_icon="📋",
    layout="wide"
)

# Paths to the files
# Mira 2.0: Uses WBP-converted assertions with dimension/layer/weight schema
# OUTPUT_FILE_PATH = WBP assertions (dimension/layer/weight)
# INPUT_FILE_PATH = Weiwei's entity file for entity cards (organized by user + meeting)
# REFERENCE_FILE_PATH = Original assertions file with responses
OUTPUT_FILE_PATH = os.path.join("docs", "ChinYew", "assertions_converted_full.jsonl")  # WBP format
INPUT_FILE_PATH = os.path.join("docs", "Weiwei", "UseUserEntity_Part1.WithUserUrl.jsonl")  # Entity context (by user)
REFERENCE_FILE_PATH = os.path.join("docs", "ChinYew", "Assertions_genv2_for_LOD1126part1.jsonl")  # For responses
MAPPING_DB_PATH = os.path.join("docs", "ChinYew", "utterance_entity_mapping.json")  # Utterance mapping
ANNOTATION_SAVE_PATH = os.path.join("docs", "annotations_mira2_temp.json")
ANNOTATION_EXPORT_PATH = os.path.join("docs", "annotated_output_mira2.jsonl")
...
def get_assertion_reasoning(assertion):
    """Get reasoning from assertion, handling multiple formats.
    
    Old format: {'reasoning': {'reason': '...', 'source': '...'}}
    Mid format: {'justification': {'reason': '...', 'sourceID': '...'}}
    WBP format: {'rationale': {...}, 'sourceID': '...'} (sourceID at top level)
    """
    if 'rationale' in assertion:
        return assertion['rationale']  # WBP format
    elif 'justification' in assertion:
...
def get_assertion_source(assertion):
    """Get source reference from assertion, handling multiple formats.
    
    Old format uses 'source' field with descriptive text (inside reasoning).
    Mid format uses 'sourceID' field inside justification.
    WBP format uses 'sourceID' at the top level of assertion.
    """
    # WBP format: sourceID is at the top level
    if 'sourceID' in assertion:
        return assertion['sourceID']
    
    # Old/mid formats: sourceID or source is inside reasoning/justification
...
def is_source_id_format(assertion):
    """Check if assertion uses sourceID format (entity IDs or source references)."""
    # WBP format: sourceID at top level
    if 'sourceID' in assertion:
        return True
    # Old/mid formats: sourceID inside reasoning/justification
    reasoning = get_assertion_reasoning(assertion)
    return 'sourceID' in reasoning


def get_assertion_dimension_info(assertion):
    """Get dimension information from WBP-format assertion.
    
    Returns dict with: dimension, dimension_name, layer, weight
    """
    return {
        'dimension': assertion.get('dimension', ''),
        'dimension_name': assertion.get('dimension_name', ''),
        'layer': assertion.get('layer', ''),
        'weight': assertion.get('weight', 0)
    }


def get_assertion_quality(assertion):
    """Get quality assessment from WBP-format assertion.
    
    Returns dict with: is_well_formed, is_testable, issues
    """
    qa = assertion.get('quality_assessment', {})
    return {
        'is_well_formed': qa.get('is_well_formed', None),
        'is_testable': qa.get('is_testable', None),
...
def main():
    ...
    st.title("✨ Mira 2.0 - WBP Assertion Viewer")
    st.caption("📊 Viewing WBP-converted assertions with dimension/layer/weight classification")
    
    # Compact data source info
    st.caption(f"📂 Data: `{os.path.basename(OUTPUT_FILE_PATH)}` | Entities: `{os.path.basename(INPUT_FILE_PATH)}`")

    # Load Data early so we can show progress and populate command center
    output_data = load_data(OUTPUT_FILE_PATH)  # WBP assertions
    input_data = load_data(INPUT_FILE_PATH)    # Weiwei entity data (by user + meeting)
    reference_data = load_data(REFERENCE_FILE_PATH)  # For responses
    
    # Load mapping DB for utterance -> entity index lookup
    mapping_db = {}
    if os.path.exists(MAPPING_DB_PATH):
        with open(MAPPING_DB_PATH, 'r', encoding='utf-8') as f:
            mapping_db = json.load(f)
    
    # Build utterance -> response map from reference file
    response_map = {}
    for item in reference_data:
        utt = item.get('utterance', '')
        if utt and 'response' in item:
            response_map[utt] = item['response']
    
    # Build utterance -> list of input_data indices map from Weiwei file
    # (same meeting can have different users, so we need to track all)
    input_utterance_map = {}  # {utterance: [list of indices]}
    for i, item in enumerate(input_data):
        utt = item.get('UTTERANCE', {}).get('text', '')
        if utt:
            if utt not in input_utterance_map:
                input_utterance_map[utt] = []
            input_utterance_map[utt].append(i)

    if not output_data:
        st.error(f"Could not find or load data from {OUTPUT_FILE_PATH}. Please ensure the file exists.")
        return
...
- **Export** your work by clicking "📤 Export" to save to `annotated_output.jsonl`
    ...
    # Create a list of options for the sidebar based on OUTPUT data (WBP assertions)
    # Output data structure: {"utterance": "...", "assertions": [...]}
    options = []
    filtered_indices = []  # Track original indices for filtered items
    
    # Track duplicates (same user + utterance) to add differentiator
    from collections import defaultdict
    user_utterance_counts = defaultdict(int)
    user_utterance_occurrence = {}
    for i, item in enumerate(output_data):
        utterance_text = item.get('utterance', '')
        user_id = item.get('user', {}).get('id', '')
        key = (user_id, utterance_text)
        user_utterance_counts[key] += 1
    
    # Second pass to assign occurrence numbers
    ...
    for i, item in enumerate(output_data):
        ...
        user_utterance_occurrence[i] = (occurrence_tracker[key], user_utterance_counts[key])
    
    for i, item in enumerate(output_data):
        utterance_text = item.get('utterance', 'No Utterance')
        # Extract subject from utterance (text inside single quotes)
        match = re.search(r"'([^']*)'", utterance_text)
        subject = match.group(1) if match else utterance_text[:50]
        
        # Get user ID from WBP data (added from Weiwei file)
        user_id = item.get('user', {}).get('id', '')
        user_suffix = f" ({user_id})" if user_id else ""
        
        # Check if this is a duplicate and add differentiator
        occurrence, total = user_utterance_occurrence.get(i, (1, 1))
        if total > 1:
            # Get first distinctive word from response to help differentiate
            response = item.get('response', '')
            # Extract first emoji or key word from response
            response_preview = response[:20].strip().split()[0] if response else ''
            dup_marker = f"[{occurrence}/{total}]"
        else:
            dup_marker = ""
        
        # Determine status indicator based on judgment
        ...
        has_entities = utterance_text in input_utterance_map
        entity_icon = "🔗" if has_entities else "⚠️"
        ...
        if include_item:
            # Format: "#6 📕 🔗 Subject [1/3] (user_id)" for duplicates
            meeting_id = i + 1
            # Truncate subject to make room for user_id and dup marker
            max_subject_len = 20 if (user_id or dup_marker) else 32
            if dup_marker:
                options.append(f"#{meeting_id} {status} {entity_icon} {subject[:max_subject_len]} {dup_marker}{user_suffix}")
            else:
                options.append(f"#{meeting_id} {status} {entity_icon} {subject[:max_subject_len]}{user_suffix}")
            filtered_indices.append(i)
    
    # Show count of filtered results
    st.sidebar.caption(f"📊 {len(options)} of {len(output_data)} meetings (🔗 = has entity data)")
    ...
    if st.session_state.get('show_reset_current_confirm', False):
        ...
        with conf_col1:
            if st.sidebar.button("✅ Yes, Reset", key="confirm_reset_current"):
                ...
                current_utterance = output_data[selected_index].get('utterance', '')
    ...
    # Get selected output item (WBP assertions)
    output_item = output_data[selected_index]
    utterance_text = output_item.get('utterance', '')
    
    # Look up corresponding input item (entity data) using mapping
    # input_utterance_map returns a list of indices (same meeting can have multiple users)
    input_indices = input_utterance_map.get(utterance_text, [])
    input_idx = input_indices[0] if input_indices else -1
    input_item = input_data[input_idx] if input_idx >= 0 else None
    ...
    # Extract subject from utterance
    match = re.search(r"'([^']*)'", utterance_text)
    subject = match.group(1) if match else utterance_text[:50]
    
    # Find the meeting/event data from entities (if input_item available)
    entities = input_item.get('ENTITIES_TO_USE', []) if input_item else []
    meeting_event = None
    for entity in entities:
    ...
    with col_title:
        ...
        # Show user and event info
        user_data = output_item.get('user', {})
        user_display = user_data.get('displayName', 'Unknown User')
        user_id = user_data.get('id', '')
        user_info = f"👤 **{user_display}** (`{user_id}`)" if user_id else "👤 User info not available"
        st.caption(f"{user_info} | Event ID: `{meeting_event_id}`")
    ...
    # Meeting Context Section - Show only if input_item is available
    if input_item:
        st.markdown(
            """<div style='background: linear-gradient(90deg, #e3f2fd 0%, #bbdefb 100%); 
                ...
                <span style='font-size: 1.1em; font-weight: 600; color: #1565c0;'>
                    📋 Meeting Context Available (from Weiwei Entity Data)
                </span>
                <span style='color: #666; margin-left: 10px;'>
                    View entities, organizer info, and source data
                </span>
            </div>""",
            unsafe_allow_html=True
        )
        ...
        # No entity data available for this meeting
        st.warning("⚠️ No entity context available for this meeting (utterance not found in Weiwei entity file)")

    # WBP assertions always available (we're navigating by output_data)
    if output_item:
        ...
        with col2:
            ...
            if not assertions:
                ...
                # Sort assertions: Structural (S) first, then Grounding (G), then others
                # Preserve original index for annotation lookup
                def get_sort_key(item):
                    idx, assertion = item
                    dim = assertion.get('dimension', '')
                    if dim.startswith('S'):
                        # S1-S19: sort by number
                        try:
                            num = int(dim[1:])
                            return (0, num)  # Structural first
                        except:
                            return (0, 99)
                    elif dim.startswith('G'):
                        # G1-G5: sort by number
                        try:
                            num = int(dim[1:])
                            return (1, num)  # Grounding second
                        except:
                            return (1, 99)
                    else:
                        return (2, 0)  # Others last
                
                sorted_assertions = sorted(enumerate(assertions), key=get_sort_key)
                
                # Build dimension occurrence counter to add sub-indices (S4-1, S4-2, etc.)
                from collections import defaultdict
                dim_counts = defaultdict(int)
                dim_occurrence = {}  # Maps (original_idx) -> occurrence number
                for orig_idx, asrt in sorted_assertions:
                    dim = asrt.get('dimension', '')
                    dim_counts[dim] += 1
                    dim_occurrence[orig_idx] = dim_counts[dim]
                
                # Count total per dimension to know if we need sub-indices
                dim_totals = defaultdict(int)
                for _, asrt in sorted_assertions:
                    dim_totals[asrt.get('dimension', '')] += 1
                
                for original_idx, assertion in sorted_assertions:
                    level = assertion.get('level', 'unknown').lower()
                    
                    # Get dimension info for badge
                    dimension_id = assertion.get('dimension', '')
                    dimension_name = assertion.get('dimension_name', '')
                    layer = assertion.get('layer', '')
                    
                    # Add sub-index if there are multiple assertions with same dimension
                    occurrence = dim_occurrence.get(original_idx, 1)
                    total_for_dim = dim_totals.get(dimension_id, 1)
                    if total_for_dim > 1:
                        dim_display = f"{dimension_id}-{occurrence}"
                    else:
                        dim_display = dimension_id
                    
                    # Dimension badge with color: blue for Structural, purple for Grounding
                    if dimension_id.startswith('S'):
                        dim_color = "#3498db"  # Blue for Structural
                        dim_badge = f"<span style='background-color: {dim_color}; color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.8em; font-weight: bold;'>{dim_display}</span>"
                    elif dimension_id.startswith('G'):
                        dim_color = "#9b59b6"  # Purple for Grounding
                        dim_badge = f"<span style='background-color: {dim_color}; color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.8em; font-weight: bold;'>{dim_display}</span>"
                    else:
                        dim_badge = ""
                    ...
                    ann = get_annotation(utterance_text, original_idx)
                    ...
                    expander_key = f"{selected_index}_{original_idx}"
                    is_expanded = expander_key in st.session_state.expanded_assertions
                    
                    # Card-like expander for each assertion
                    # Include GPT-5 icon and dimension badge in header
                    gpt5_header = f"[GPT5:{gpt5_icon}]" if gpt5_icon else ""
                    dim_header = f"[{dimension_id}]" if dimension_id else ""
                    with st.expander(f"{judged_icon} {gpt5_header} {dim_header} {evidence_icon} {feedback_icon} :{color}[**{level.upper()}**] - {assertion.get('text', '')[:50]}...", expanded=is_expanded):
                        ...
                        with col_correct:
                            ...
                            is_good_new = st.checkbox(
                                ...
                                value=is_good, 
                                key=f"good_{selected_index}_{original_idx}"
                            )
                            if is_good_new != is_good:
                                # Auto-mark as judged when user makes a judgment
                                set_annotation(utterance_text, original_idx, is_good=is_good_new, is_judged=True, original=assertion.get('text', ''))
                                save_annotations()
                        ...
                        with col_confident:
                            ...
                            is_confident_new = st.checkbox(
                                ...
                                value=is_confident,
                                key=f"confident_{selected_index}_{original_idx}"
                            )
                            if is_confident_new != is_confident:
                                # Auto-mark as judged when user indicates confidence level
                                set_annotation(utterance_text, original_idx, is_confident=is_confident_new, is_judged=True)
                                save_annotations()
                        ...
                        new_note = st.text_area(
                            "Add your notes here",
                            value=note,
                            key=f"note_{selected_index}_{original_idx}",
                            height=80,
                            placeholder="Enter any comments about this assertion...",
                            label_visibility="collapsed"
                        )
                        if new_note != note:
                            set_annotation(utterance_text, original_idx, note=new_note)
                        ...
                        new_revision = st.text_area(
                            ...
                            key=f"revision_{selected_index}_{original_idx}",
                            height=100,
                            placeholder="Enter a revised version of this assertion if needed...",
                            label_visibility="collapsed"
                        )
                        if new_revision != revision:
                            set_annotation(utterance_text, original_idx, revision=new_revision, original=assertion.get('text', ''))
                        
                        # Collapse button to close this expander when done
                        if st.button("🔼 Done - Collapse", key=f"collapse_{selected_index}_{original_idx}", help="Close this assertion card"):
                        ...
                        if gpt5_score:
                            ...
                            with exp_col1:
                                new_exp_verified = st.checkbox(
                                    ...
                                    key=f"gpt5_exp_verify_{selected_index}_{original_idx}",
                                ...
                                if new_exp_verified != explanation_verified:
                                    ...
                                    set_annotation(utterance_text, original_idx, gpt5_verification=current_gpt5_ann)
                                    save_annotations()
                            with exp_col2:
                                if new_exp_verified:
                                ...
                                for span_idx, span in enumerate(supporting_spans):
                                    ...
                                    with st.container():
                                        ...
                                        new_span_verified = st.checkbox(
                                            ...
                                            value=span_verified,
                                            key=f"gpt5_span_verify_{selected_index}_{original_idx}_{span_idx}",
                        ...
                        if reasoning:
                            ...
                            if source:
                                    ...
                                    if entity_info:
                                        ...
                                        with st.container(border=True):
                                            ...
                                            if actual_entity_type == 'User':
                                                ...
                                                render_file_card(entity_data, key_suffix=f"inline_{original_idx}")
                                            elif actual_entity_type == 'Chat':
                                                render_chat_card(entity_data, key_suffix=f"inline_{original_idx}")
                                            elif actual_entity_type == 'Email':
                                                render_email_card(entity_data, key_suffix=f"inline_{original_idx}")
                                            elif actual_entity_type in ['ChannelMessage', 'ChannelMessageReply']:
                                                render_channel_message_card(entity_data, key_suffix=f"inline_{original_idx}")
                                            else:
                                        ...
                                        if st.button(f"🔗 View in Input Context", key=f"link_entity_{selected_index}_{original_idx}", help=f"Jump to {actual_entity_type} in Input Context"):
                                ...
                                if matched_segments:
                                    ...
                                    if st.button(f"🔍 Show Evidence", key=f"locate_{selected_index}_{original_idx}"):
                                        st.session_state["highlight_matches"] = matched_segments
                                        st.session_state["active_assertion_index"] = original_idx
                                        st.rerun()
                                else:
                                ...
                                if st.session_state.get("active_assertion_index") == original_idx and st.session_state.get("highlight_matches"):
                                    ...
                                    with col2:
                                        if st.button("❌", key=f"clear_{selected_index}_{original_idx}", help="Clear Highlight"):
